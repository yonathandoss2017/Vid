...
function getGlobal($Date1, $Date2, $idAccM){
	...
	$Heads[18] = 'Paula';
	$Heads[27567] = 'Maria José';
	$Heads[28464] = 'Elena Lafuente';
	...
	foreach($Heads as $idHead => $HeadName){
		...
		$SHQuery = '';
		$sql = "SELECT * FROM user WHERE manager_id = $idHead AND roles LIKE '%SUB_HEAD%'";
		$query2 = $db2->query($sql);
		if($db2->num_rows($query2) > 0){
			while($SH = $db2->fetch_array($query2)){
				$idSub = $SH['id'];
				$SHQuery .= "OR manager_id = $idSub ";
			}
		}
		
		$sql = "SELECT * FROM user WHERE (manager_id = $idHead $SHQuery) AND status = 1";
		$query2 = $db2->query($sql);
		if($db2->num_rows($query2) > 0){
			while($AccM = $db2->fetch_array($query2)){
				$idAccM = $AccM['id'];
