    ...
    while ($AccM = $db3->fetch_array($query)) {
        ...
        if (checkActiveSales($idUser)) {
            ...
            $sql = "SELECT COUNT(DISTINCT(idCampaing)) FROM reports
                ...
                WHERE campaign.sales_manager_id = '$idUser' AND campaign.type = 1 AND reports.Date BETWEEN '$Date5' AND '$Date1' AND reports.Impressions > 0";
            $ActiveDeals = $db->getOne($sql);

            $sql = "SELECT COUNT(DISTINCT(idCampaing)) FROM reports
                INNER JOIN campaign ON campaign.id = reports.idCampaing
                WHERE campaign.sales_manager_id = '$idUser' AND campaign.type = 2 AND reports.Date BETWEEN '$Date5' AND '$Date1' AND reports.Impressions > 0";
            $ActiveCamp = $db->getOne($sql);

            $sql = "SELECT COALESCE(SUM(Revenue), 0) Revenue FROM reports
            INNER JOIN campaign ON campaign.id = reports.idCampaing
            WHERE campaign.sales_manager_id = '$idUser' AND reports.Date BETWEEN '$FirstDay' AND '$LastDate'";
            $CurrentRevenue = $db->getOne($sql);

            $sql = "SELECT COALESCE(SUM(Revenue), 0) Revenue FROM reports
            INNER JOIN campaign ON campaign.id = reports.idCampaing
            WHERE campaign.sales_manager_id = '$idUser' AND reports.Date BETWEEN '$Date1' AND '$Date1'";
            $YesterdayRevenue = $db->getOne($sql);

            $sql = "SELECT COALESCE(SUM(Revenue), 0) Revenue FROM reports
            INNER JOIN campaign ON campaign.id = reports.idCampaing
            WHERE campaign.sales_manager_id = '$idUser' AND reports.Date BETWEEN '$Date3' AND '$Date3'";
            ...
            if ($Goal > $CurrentRevenue) {
                if (0 <= $DaysToFinishMonth) {
                    $GoalPerDay = ($Goal - $CurrentRevenue);
                } else {
...
$sql = "SELECT 
        ...
        INNER JOIN user ON campaign.sales_manager_id = user.id
