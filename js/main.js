;(function($) {
	...
	function scrollStuff() {
		var filter_top = $('.filter-dropdown').offset().top,
			scroll_top = $(window).scrollTop();

		$('.filter-dropdown .fd-menu').css('top', filter_top - scroll_top + 45)
	}
	...
	function openDropdownMenus(e){
		...
		if ($(this).text().toLowerCase() == 'include') {
			$button.find('button').removeClass('exclude').addClass('include');
			$(this).closest('.flex-top').find('.rm-box').removeClass('exclude').addClass('include');
		} else if($(this).text().toLowerCase() == 'exclude') {
			$button.find('button').removeClass('include').addClass('exclude');
			$(this).closest('.flex-top').find('.rm-box').removeClass('include').addClass('exclude');
		}
	}
