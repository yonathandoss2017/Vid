<?php	
	...
	//$Date = date('Y-m-d');
	$Date1 = date('2018-09-30');
	$Date2 = date('2018-09-30');
	/*
	$headers = array(
	...
	foreach($decoded_result->data->entries as $entry){
		...
		if($LKQDid == '44422'){
			...
			if($inserta == 1){
				...
				if($idStat > 0){
					$sql = "UPDATE " . STATS . " SET Impressions = '$Impressions', Opportunities = '$Opportunities', Revenue = '$Revenue', Clicks = '$Clicks', lastUpdate = '$lastUpdate' WHERE id = '$idStat' LIMIT 1";
					$db->query($sql);
				}else{
					$sql = "INSERT INTO " . STATS . " (idUser, idTag, Impressions, Opportunities, Revenue, Clicks, timeAdded, lastUpdate, Date) VALUES ('$idUser', '$idTag', '$Impressions', '$Opportunities', '$Revenue', '$Clicks', '$timeAdded', '$lastUpdate', '$Date')";
					$db->query($sql);
				}
			}
		}
