	...
	if($db->num_rows($query) > 0){
		...
		while($Ads = $db->fetch_array($query)){
			...
			if($Type != 5){
				...
				$Code = str_replace('#LKQDid',$LKQDid,$Code);
			}else{
				$Code = "// custome code \n\n" . $Ads['CCode'];
			}
...
function notifySiteEric($idSite, $Aprove, $Message = ''){
	global $db, $mail;
	
	$sql = "SELECT a.Email, a.Name, s.sitename, s.siteurl FROM " . SITES . " s 
	INNER JOIN " . USERS . " u ON u.id = s.idUser 
	INNER JOIN " . ACC_MANAGERS . " a ON a.id = u.AccM 
	WHERE s.id = '$idSite' LIMIT 1";
	
	$query = $db->query($sql);
	if($db->num_rows($query) > 0){
		$AccMData = $db->fetch_array($query);
		$AccMEmail = $AccMData['Email'];
		$AccMName = $AccMData['Name'];
		$SiteName = $AccMData['sitename'];
		$SiteURL = $AccMData['siteurl'];
	}else{
		$AccMEmail = 'federico.izuel@vidoomy.com';
		$AccMName = 'No Manager';
	...
	$mail->setFrom('alta@vidoomy.com', 'Vidoomy');
	$mail->addReplyTo('alta@vidoomy.com', 'Vidoomy');
	$mail->addAddress($AccMEmail, $AccMName);
	...
	if($Aprove){
		$Subject = $SiteName . ': Sitio Aprobado!';
	}else{
		$Subject = $SiteName . ': Sitio Rechazado';
	}
	...
	if($Aprove){
		$message .= "<p>$AccMName,<br><br>El sitio $SiteURL ha sido aprobado. Los anuncios ya funcionarán con normalidad y podrá empezar a generar ganancias.<br/><br/>Recuerda quitar el Modo Test si lo has utilizado y ya no lo necesita!</p>";
	}else{
		$message .= "<p>$AccMName,<br><br>El sitio $SiteURL ha sido rechazado por las siguientes razones:<br/><br/>$Message</p>";
	}
	$message .= "</body></html>";
