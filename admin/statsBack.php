<?php	
	...
	//REVENUE YESTERDAY
	$sql = "SELECT idTag, SUM(Impressions) AS Impressions, SUM(Revenue) AS Revenue FROM " . STATS .  " WHERE Date = '$Yesterday' GROUP BY idTag";
	$query = $db->query($sql);
	...
	//REVENUE TODAY
	$sql = "SELECT idTag, SUM(Impressions) AS Impressions, SUM(Revenue) AS Revenue FROM " . STATS .  " WHERE Date = '$Today' GROUP BY idTag";
	$query = $db->query($sql);
	$ImpressionsToday = 0;
	if($db->num_rows($query) > 0){
		while($dataTag = $db->fetch_array($query)){
	...
	$LastDay = date('Y-m-t');
	$sql = "SELECT idTag, SUM(Impressions) AS Impressions, SUM(Revenue) AS Revenue FROM " . STATS .  " WHERE Date BETWEEN '$FirstDay' AND '$LastDay' GROUP BY idTag";
	$query = $db->query($sql);
	if($db->num_rows($query) > 0){
		while($dataTag = $db->fetch_array($query)){
			$Revenuevalue = $TagList[$dataTag['idTag']]['Revenue'];
					...
					<div class="bx-bd">
						<div class="bx-pd">
							<ul class="lst-tbs b-fx mb2">
								...
								<!--<li><a href="#" class="fa-flag">Website</a></li>-->
											...
											if($view == 2){
												...
												if($db->num_rows($query) > 0){
													while($User = $db->fetch_array($query)){
														...
														$sql = "SELECT idTag, SUM(Impressions) AS Impressions, SUM(Revenue) AS Revenue FROM " . STATS .  " WHERE idUser = '$idUser' AND Date BETWEEN '$FirstDay' AND '$LastDay' GROUP BY idTag";
														$query2 = $db->query($sql);
														//exit(0);
														$Revenue = 0;
														$Impressions = 0;
														...
														?><tr>
															<td data-title="Fecha"> <?php echo $User['user'];?></td>
															<!--<td data-title="Clicks">2,043 <i class="fa-caret-up"></i></td>
															<td data-title="Altas">124 <i class="fa-caret-down"></i></td>-->
															<td data-title="Impresiones"><?php echo number_format($Impressions, 0, ',', '.'); ?></td>
												...
												for($d = $Days; $d >= 1; $d--){
													...
													$sql = "SELECT idTag, SUM(Impressions) AS Impressions, SUM(Revenue) AS Revenue FROM " . STATS .  " WHERE Date = '$date' GROUP BY idTag";
													$query = $db->query($sql);
													$Impressions = 0;
													$Revenue = 0;
													$Coste = 0;
