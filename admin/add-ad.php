				...
				if(isset($_POST['save'])){
					...
					if($_POST['save'] != ''){
						...
						if($Type >= 10 && $Type < 20){
							...
							if($NewAdUnits == ''){
								$sigue = false;
								$ErrorNoAdUnits = 'No se puede crear un nuevo Anuncio de Display sin Ad Units.';
							}
						}else{
						...
						if($sigue){
							...
							$sql = "INSERT INTO ad 
							(id, website_id, created_by, ad_type_id, lkqdid, divid, width, height, close, dfp, override, height_a, sposition, ccode, status, created_at) 
							VALUES 
							($idAD, $siteId, 1, '$Type', '$idLkqd', '$divID', '$Width', '$Height', '$Close', '$DFP', '$Override', '$AA', '$Spos', \"$CCode\", 1, '$created_at')";
							$db2->query($sql);
							//echo $sql;
							//exit(0);
...
<body>
							...
							<div class="bx-bd">
								...
								<div class="bx-pd">
									<form action="" method="post" class="frm-adrsit">
										...
										<div class="botnr-cn">
											<input type="submit" class="fa-save" value="Crear Anuncio" name="save" />
										</div>
									</form>
								<div>		
    ...
    <style type="text/css">
	    ...
	    <?php 
		    if($Type <= 9){ echo ".displayads, .customcode {display:none;} "; }
		    elseif($Type <= 20){ echo ".customcode, .videoads {display:none;} "; }
		    else{ echo ".displayads, .videoads {display:none;} "; }
		    
		    
		    if($Type <= 2){ echo ".slider-type{display:none;}"; } else { echo ".no-slider-type{display:none;}"; } ?>
	</style>
		...
		jQuery(document).ready(function($){
			$("#type").change(function(){
				if($("#type").val() <= 9){
					...
					$("#displayadload").load("new-display-ad.php?pla="+selPlatform, function() { 
