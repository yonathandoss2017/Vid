<?php	
	...
	if(@$_SESSION['Type'] == 3){
		if(isset($_GET['idp'])){
			$idPlacement = intval($_GET['idp']);
			
			$sql = "SELECT * FROM  " . PLACEMENTS . " WHERE id = '$idPlacement' LIMIT 1";
			$query = $db->query($sql);
			if($db->num_rows($query) > 0){
				$PlacementData = $db->fetch_array($query);
				
				$idBidder = $PlacementData['idBidder'];
				$Name = $PlacementData['Name'];
				$Platform = $PlacementData['Platform'];
				$Default = $PlacementData['isDefault'];
				$Tam = $PlacementData['Size'];
				$ID = $PlacementData['idZone'];
				
				if($PlacementData['Custom'] != ''){
					$arC = explode('[]',$PlacementData['Custom']);
					$Custom1 = $arC[0];
					$Custom2 = $arC[1];
				}
				
				$sqlB = "SELECT * FROM " . BIDDERS . " WHERE id = '$idBidder' LIMIT 1";
				$queryB = $db->query($sqlB);
				if($db->num_rows($query) > 0){
					$BidderData = $db->fetch_array($queryB);
					...
					if(isset($_POST['save'])){
						...
						*/
						...
						if($Sigue){
							...
							$sql = "UPDATE " . PLACEMENTS . " SET Name = '$Name', Platform = '$Platform', isDefault = '$Default', idZone = '$ID', Size = '$Tam', Custom = '$Custom' WHERE id = '$idPlacement' LIMIT 1";
							$db->query($sql);
					...
					<main class="clmc12 c-flx1">
						...
						<div class="bx-cn bx-shnone">
							<div class="bx-hd dfl b-fx">
								<div class="titl">Editar Placement <?php echo $Name; ?> de <?php echo $Bidder; ?></div>
							</div>
							<div class="bx-bd">
								<div class="bx-pd">
									<div id="formadd">
									<form action="" method="post" class="frm-adrsit">
										<div class="bx-hd dfl b-fx">
											<div class="titl">Editar Placement</div>
										</div>
										<div class="clsd-fx">
											<div class="clmd06">
												<!--<Nombre>-->
