<?php
	...
	if(isset($_POST['user'])){
		...
		$PUser = my_clean($_POST['user']);
		if(strlen($_POST['pass']) == 32){
			$PPass = my_clean($_POST['pass']);
		}else{
			$PPass = md5($_POST['pass']);
		}
		$sql = "SELECT id FROM " . ACC_MANAGERS. " WHERE Email = '$PUser' AND Password = '$PPass' AND Deleted = 0 LIMIT 1 ";
					...
					<div class="bx-bd">
						<div class="bx-pd">
							<ul class="lst-tbs b-fx mb2">
								...
								<li class="b-rt"><a href="add-user.php" class="fa-plus-circle" style="display:inline-block;">Añadir Publisher</a><?php
								if($_SESSION['Type'] == 3){
								?>&nbsp;&nbsp;&nbsp;<a href="confirm.php" class="fa-check-circle" style="display:inline-block;">Publishers Pendientes de Aprobaci&oacute;n (<?php
								$sql = "SELECT COUNT(id) FROM " . USERS . " WHERE AccM = 15";
								echo $db->getOne($sql);
								?>)</a>
								&nbsp;&nbsp;&nbsp;<a href="index.php?a=1" class="fa-times-circle" style="display:inline-block;">Publishers Aceptados</a>
								&nbsp;&nbsp;&nbsp;<a href="confirm.php?den=1" class="fa-times-circle" style="display:inline-block;">Publishers Denegados</a>
								<?php } ?>
								</li>
							</ul>					
							<!--<table>-->
