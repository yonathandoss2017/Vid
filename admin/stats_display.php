<?php	
	...
	if($_SESSION['Type'] == 3){
		...
		$sql = "SELECT SUM(Amount) FROM " . PREBID_REVENUE .  " INNER JOIN " . USERS .  " ON " . USERS .  ".id = " . PREBID_REVENUE .  ".idUser WHERE " . USERS .  ".AccM = '$idAccM' AND " . PREBID_REVENUE .  ".Date = '$Yesterday'";
	}
	$RevenueYesterday = $db->getOne($sql);
	...
	if($_SESSION['Type'] == 3){
		...
		$sql = "SELECT SUM(Amount) FROM " . PREBID_REVENUE .  " INNER JOIN " . USERS .  " ON " . USERS .  ".id = " . PREBID_REVENUE .  ".idUser WHERE " . USERS .  ".AccM = '$idAccM' AND " . PREBID_REVENUE .  ".Date = '$Today'";
	}
	$RevenueToday = $db->getOne($sql);
	
	if($_SESSION['Type'] == 3){
		$sql = "SELECT SUM(formatLoads) FROM " . PREBID_FL .  " WHERE Date = '$Today'";
	}else{
		$sql = "SELECT SUM(formatLoads) FROM " . PREBID_FL .  " INNER JOIN " . USERS .  " ON " . USERS .  ".id = " . PREBID_FL .  ".idUser WHERE " . USERS .  ".AccM = '$idAccM' AND " . PREBID_FL .  ".Date = '$Today'";
	}
	$ImpressionsToday = $db->getOne($sql);
	...
	if($_SESSION['Type'] == 3){
		...
		$sql = "SELECT SUM(Amount) FROM " . PREBID_REVENUE .  " INNER JOIN " . USERS .  " ON " . USERS .  ".id = " . PREBID_REVENUE .  ".idUser WHERE " . USERS .  ".AccM = '$idAccM' AND " . PREBID_REVENUE .  ".Date BETWEEN '$FirstDay' AND '$LastDay'";
	}
	$RevenueThisMonth = $db->getOne($sql);
...
<head>
    <meta charset="utf-8">
    <title>Vidoomy - Display Stats</title>
								...
								<div class="dropdown-menu">											
									<div class="fs-dropdown-options bx-cn">
										<span class="fs-dropdown-group">Seleccionar Fecha</span>
											<button type="button" class="fs-dropdown-item" data-value="1" onclick="location.href='stats_display.php?range=today<?php echo $andv; ?>'">Estádisticas de Hoy</button>
											<button type="button" class="fs-dropdown-item" data-value="2" onclick="location.href='stats_display.php?range=yesterday<?php echo $andv; ?>'">Estádisticas de Ayer</button>
											<button type="button" class="fs-dropdown-item" data-value="3" onclick="location.href='stats_display.php?range=thismonth<?php echo $andv; ?>'">Estádisticas mes Actual</button>
											<button type="button" class="fs-dropdown-item" data-value="4" onclick="location.href='stats_display.php?range=lastmonth<?php echo $andv; ?>'">Estádisticas último Mes</button>
											<div class="frm-fltr">
												<form action="stats_display.php?range=custom<?php echo $andv; ?>" method="post">
					...
					<div class="bx-bd">
						<div class="bx-pd">
							<ul class="lst-tbs b-fx mb2">
								<li<?php echo $view1; ?>><a href="stats_display.php?<?php echo $andr; ?>view=1">Fecha</a></li>
								<!--<li<?php echo $view2; ?>><a href="stats_display.php?<?php echo $andr; ?>view=2" class="fa-android">Sitios</a></li>
								<li<?php echo $view3; ?>><a href="stats_display.php?<?php echo $andr; ?>view=3" class="fa-globe">Zonas</a></li>
								<li><a href="#" class="fa-street-view">Paises</a></li>-->
								<li<?php echo $view5; ?>><a href="stats_display.php?<?php echo $andr; ?>view=5" class="fa-globe">Publisher</a></li>
								<li<?php echo $view6; ?>><a href="stats_display.php?<?php echo $andr; ?>view=6" class="fa-globe">Bidder</a></li>
							</ul>						
							<!--<table>-->
							<div class="tbl-cn">
								<table id="tbl-estats">
									...
									<tbody>
											...
											if($view == 2){
												...
												if($db->num_rows($query) > 0){
													while($dataTag = $db->fetch_array($query)){
														...
														if($TagList[$dataTag['idTag']]['RevenueType'] == 1){
															$CosteThis = $dataTag['Revenue'] * $Revenuevalue / 100;
														}else{
															$CosteThis = $dataTag['Impressions'] * $Revenuevalue;
														}*/
												...
												if($db->num_rows($query) > 0){
													while($User = $db->fetch_array($query)){
														...
														$Coste = $Revenue / 2;
														$sql = "SELECT SUM(Impressions) FROM " . PREBID_REVENUE .  " WHERE idUser = '$idUser' AND Date BETWEEN '$FirstDay' AND '$LastDay'";
														$Impressions = $db->getOne($sql);
														$sql = "SELECT SUM(formatLoads) FROM " . PREBID_FL .  " WHERE idUser = '$idUser' AND Date BETWEEN '$FirstDay' AND '$LastDay'";
														$formatLoads = $db->getOne($sql);
														
														$TotalFormatLoads += $formatLoads;
														$TotalImpressions += $Impressions;
														$TotalRevenue += $Revenue;
														$TotalCoste += $Coste;
														
														if($formatLoads > 0 && $Revenue > 0){
															$eCpm = $Revenue / $formatLoads * 1000;
															$eCpmShow = $DE . number_format($eCpm, 2, ',', '.') . $DEb;
														}else{
															$eCpmShow = 'NA';
														}
														...
														?><tr>
															...
															<td data-title="FormatLoads"><?php echo $formatLoads; ?></td>
															<td data-title="Impresiones"><?php echo $Impressions; ?></td>
												...
												foreach($DisplayBidders as $BidderId => $BidderName){
													$sql = "SELECT SUM(Amount) FROM " . PREBID_REVENUE_BIDDER .  " WHERE Bidder = '$BidderId' AND Date BETWEEN '$FirstDay' AND '$LastDay'";
													$Revenue = $db->getOne($sql);
													$Coste = $Revenue / 2;
													$sql = "SELECT SUM(Impressions) FROM " . PREBID_REVENUE_BIDDER .  " WHERE Bidder = '$BidderId' AND Date BETWEEN '$FirstDay' AND '$LastDay'";
													$Impressions = $db->getOne($sql);
													...
													?><tr>
														<td data-title="Fecha"><?php  echo $BidderName; ?></td>
														<td data-title="FormatLoads">NA</td>
														<td data-title="Impresiones"><?php echo $Impressions; ?></td>
												...
												for($d = $Days; $d >= 1; $d--){
													...
													if($_SESSION['Type'] == 3){
														...
														$sql = "SELECT SUM(Amount) FROM " . PREBID_REVENUE .  " INNER JOIN " . USERS .  " ON " . USERS .  ".id = " . PREBID_REVENUE .  ".idUser WHERE " . USERS .  ".AccM = '$idAccM' AND " . PREBID_REVENUE .  ".Date = '$date'";
													}
													$Revenue = $db->getOne($sql);
													$Coste = $Revenue / 2;
													...
													if($_SESSION['Type'] == 3){
														$sql = "SELECT SUM(formatLoads) FROM " . PREBID_FL .  " WHERE Date = '$date'";
													}else{
														$sql = "SELECT SUM(formatLoads) FROM " . PREBID_FL .  " INNER JOIN " . USERS .  " ON " . USERS .  ".id = " . PREBID_FL .  ".idUser WHERE " . USERS .  ".AccM = '$idAccM' AND " . PREBID_FL .  ".Date = '$date'";
													}													
													$formatLoads = $db->getOne($sql);
													...
													if($_SESSION['Type'] == 3){
														$sql = "SELECT SUM(Impressions) FROM " . PREBID_REVENUE .  " WHERE Date = '$date'";
													}else{
														$sql = "SELECT SUM(Impressions) FROM " . PREBID_REVENUE .  " INNER JOIN " . USERS .  " ON " . USERS .  ".id = " . PREBID_REVENUE .  ".idUser WHERE " . USERS .  ".AccM = '$idAccM' AND " . PREBID_REVENUE .  ".Date = '$date'";
													}
													$Impressions = $db->getOne($sql);
													...
													?><tr>
														...
														<td data-title="FormatLoads"><?php echo number_format($formatLoads, 0, ',', '.'); ?></td>
														<td data-title="Impressions"><?php echo number_format($Impressions, 0, ',', '.'); ?></td>
														<td data-title="eCPM"><?php echo $eCpmShow; ?></td>
														<td data-title="Ganancias"><strong><span><?php echo $DE . number_format($Revenue, 2, ',', '') . $DEb; ?></span></strong></td>
														<td data-title="Coste"><strong><span><?php echo $DE . number_format($Coste, 2, ',', '') . $DEb; ?></span></strong></td>
														<td data-title="Profit"><strong><span><?php echo $DE . number_format($Profit, 2, ',', '') . $DEb; ?></span></strong></td>
										...
										<tfoot>
										<tr>
											<th colspan="8"><span>Totales: <!--Clicks:-->Impresiones:</span> <strong><?php echo number_format($TotalImpressions, 0, ',', '.'); ?></strong></span> <span class="txspr">·</span> <!--<span>% de Conversiones: <strong>1.19%</strong></span> <span class="txspr">·</span>--> <span>CPM: <strong><?php echo $totaleCpmShow; ?></strong></span> <span class="txspr">·</span> <span>Revenue: <strong><?php echo $DE . number_format($TotalRevenue, 2, ',', '.') . $DEb; ?></strong></span><span class="txspr">·</span> <span>Coste: <strong><?php echo $DE . number_format($TotalCoste, 2, ',', '.') . $DEb; ?></strong></span></th>
										</tr>
										</tfoot>
									</tbody>
								</table>
