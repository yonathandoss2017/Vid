	...
	if($ImportData !== false){
		...
		foreach($ImportData as $DataK => $DataL){
			...
			if($N > 0 && $Last === false){
				...
				if($inserta == 1){
					...
					$Values .= "$Coma ('$idUser', '$idTag', '$idSite', '$idDomain', '$idCountry', '$Impressions', '$Opportunities', '$formatLoads', '$Revenue', '$Coste', '$ExtraprimaP', '$Extraprima', '$Clicks', '$Wins',  '$adStarts', '$FirstQuartiles', '$Midpoints', '$ThirdQuartiles', '$CompletedViews', '$timeAdded', '$lastUpdate', '$DateFrom', '$Hour')";
					$Coma = ",";
					if($Nins == 1000){
						//exit(0);
		...
		if($Ni > 0){
			...
			while($Da = $db->fetch_array($query)){
				...
				$Values .= "$Coma ('$idUser', '$idTag', '$idSite', '$idDomain', '$idCountry', '$Impressions', '$Opportunities', '$formatLoads', '$Revenue', '$Coste', '$ExtraprimaP', '$Extraprima', '$Clicks', '$Wins',  '$adStarts', '$FirstQuartiles', '$MidViews', '$ThirdQuartiles', '$CompletedViews', '$DateFrom')";
				$Coma = ", ";
