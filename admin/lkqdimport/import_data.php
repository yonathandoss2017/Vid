		...
		while($MoreResults == 1){
			...
			foreach($decoded_result->data->entries as $entry){
				...
				if($inserta == 1){
					...
					$sql = "SELECT id FROM reports_resume WHERE 
					idUser = '$idUser' 
					AND idTag = '$idTag' 
					AND Domain = '$idDomain' 
					...
					if($idStat > 0){
						...
						$sql = "UPDATE reports_resume SET Impressions = '$Impressions', Opportunities = '$Opportunities', formatLoads = '$formatLoads', Revenue = '$Revenue', Coste = '$Coste', Extraprima =  (ExtraprimaP * $Revenue / 100), Clicks = '$Clicks', lastUpdate = '$lastUpdate' WHERE id = '$idStat' LIMIT 1";
						$db->query($sql);
						$Nu++;
					}else{
						$ExtraprimaP = rand(27, 33);
						$Extraprima	= $ExtraprimaP * $Revenue / 100;
	
						$sql = "INSERT INTO reports_resume (idUser, idTag, idSite, Domain, Country, Impressions, Opportunities, formatLoads, Revenue, Coste, ExtraprimaP, Extraprima, Clicks, Wins, adStarts, FirstQuartiles, MidViews, ThirdQuartiles, CompletedViews, timeAdded, lastUpdate, Month, Date) VALUES ('$idUser', '$idTag', '$idSite', '$idDomain', '$idCountry', '$Impressions', '$Opportunities', '$formatLoads', '$Revenue', '$Coste', '$ExtraprimaP', '$Extraprima', '$Clicks', '$Wins',  '$adStarts', '$FirstQuartiles', '$Midpoints', '$ThirdQuartiles', '$CompletedViews', '$timeAdded', '$lastUpdate', '$Month', '$Date')";
						$db->query($sql);
						$Ni++;
					}
				}else{
