<?php	
	...
	$DateS = date('Y-m-d', time() - 18000);
	//$DateS = '2019-05-27';
	$DateA = explode('-',$DateS);
	$Month = $DateA[0] . '-' . $DateA[1];
	

	$sql = "TRUNCATE reports_consolidate_day";
	$db->query($sql);
	...
	$MoreResults = 1;
	while($MoreResults == 1){
		$decoded_result = getResultsDay($DateS, $Offset);
		...
		foreach($decoded_result->data->entries as $entry){
			...
			if($inserta == 1){
				...
				$sql = "INSERT INTO reports_consolidate_day (idUser, idTag, idSite, Domain, Country, Impressions, Opportunities, formatLoads, Revenue, Coste, ExtraprimaP, Extraprima, Clicks, Wins, adStarts, FirstQuartiles, MidViews, ThirdQuartiles, CompletedViews, timeAdded, lastUpdate, Month, Date) VALUES ('$idUser', '$idTag', '$idSite', '$idDomain', '$idCountry', '$Impressions', '$Opportunities', '$formatLoads', '$Revenue', '$Coste', '$ExtraprimaP', '$Extraprima', '$Clicks', '$Wins',  '$adStarts', '$FirstQuartiles', '$Midpoints', '$ThirdQuartiles', '$CompletedViews', '$timeAdded', '$lastUpdate', '$Month','$Date')";
				$db->query($sql);
				$Ni++;
	...
	$sql = "DELETE FROM reports_resume WHERE Date = '$DateS'";
	$db->query($sql);

	echo date('Y-m-d H:i:s');
	echo "\n";

	$sql = "SELECT COUNT(*) FROM reports_consolidate_day";
	$Tot = $db->getOne($sql);
	$Count = 0;
	$ST = 0;
	while($Count < $Tot){
		$sql = "SELECT * FROM reports_consolidate_day ORDER BY id ASC LIMIT $Count , 1000";
		$query = $db->query($sql);
		while($Row = $db->fetch_array($query)){
			...
			$sql2 = "INSERT INTO reports_resume (idUser, idTag, idSite, Domain, Country, Impressions, Opportunities, formatLoads, Revenue, Coste, ExtraprimaP, Extraprima, Clicks, Wins, adStarts, FirstQuartiles, MidViews, ThirdQuartiles, CompletedViews, Month, Date) VALUES ('$idUser', '$idTag', '$idSite', '$idDomain', '$idCountry', '$Impressions', '$Opportunities', '$formatLoads', '$Revenue', '$Coste', '$ExtraprimaP', '$Extraprima', '$Clicks', '$Wins',  '$adStarts', '$FirstQuartiles', '$Midpoints', '$ThirdQuartiles', '$CompletedViews', '$Month', '$Date')";
			$db->query($sql2);
		}
