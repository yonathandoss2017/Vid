...
function calcPercents($Perc , $Impressions, $Complete){
	...
	$HourToCopy = 13;
	$HourDestiny = 23;
	$PercentVariation = 0.66;
	
	echo "Copy hour $HourToCopy to $HourDestiny on $Date (Variation: $PercentVariation) \n";
	...
	$sql = "DELETE FROM $TablaName WHERE Date = '$DateFrom' AND Player = 1 AND Hour = $HourDestiny";
	$db->query($sql);
    ...
    FROM $TablaName WHERE Date = '$DateFrom' AND Hour = '$HourToCopy' AND idUser > 0 AND Player = 1
	...
	while($Da = $db->fetch_array($query)){
		...
		$Values .= "$Coma ('$idUser', '$idTag', '$idSite', '$idDomain', '$idCountry', '1', '$Impressions', '$Opportunities', '$formatLoads', '$Revenue', '$RevenueEur', '$Coste', '$CosteEur', '$ExtraprimaP', '$Extraprima', '$Clicks', '$Wins',  '$adStarts', '$FirstQuartiles', '$MidViews', '$ThirdQuartiles', '$CompletedViews', '$DateFrom', '$HourDestiny')";
		$Coma = ", ";
