...
function calcPercents($Perc , $Impressions, $Complete){
	...
	*/
	...
	$sql = "SELECT reports.* FROM reports WHERE reports.idCampaing = $idCampaing AND reports.Date = '$Date1'";
	//echo "\n\n";
	...
	if($db->num_rows($query) > 0){
		while($Row = $db->fetch_array($query)){
			...
			$sql = "SELECT id FROM reports WHERE Date = '$Date2' AND Hour = '$Hour' AND idCampaing = $idCampaing";
			$idStat = $dbd->getOne($sql);
			if($idStat > 0){
