							...
							<div class="tbl-cn">
								<table id="tbl-estats">
									...
									<tbody>
											...
											if($view == 2 && $ShowPL === true){
												...
												if($db->num_rows($query) > 0){
													while($Site = $db->fetch_array($query)){
														...
														$sql = "SELECT idTag, SUM(Impressions) AS Impressions, SUM(Revenue) AS Revenue FROM " . STATS .  " WHERE idUser = '" . $_SESSION['login'] . "' AND idSite = '$idSite' AND Date BETWEEN '$FirstDay' AND '$LastDay' GROUP BY idTag";
														$query2 = $db->query($sql);
														//exit(0);
														$ThisRevenue = 0;
														$ThisImpressions = 0;
														...
														?><tr>
															<td data-title="Fecha"> <?php echo $Site['sitename'];?></td>
															<!--<td data-title="Clicks">2,043 <i class="fa-caret-up"></i></td>
															<td data-title="Altas">124 <i class="fa-caret-down"></i></td>--><?php
														if($ShowCPM){
										...
										<tfoot>
										<tr>
											<th colspan="4"><span>Totales: <!--Clicks:--><?php if($ShowCPM){ ?>Impresiones:</span> <strong><?php echo number_format($TotalImpressions, 0, ',', '.'); ?></strong></span> <span class="txspr">·</span> <!--<span>% de Conversiones: <strong>1.19%</strong></span> <span class="txspr">·</span>--> <span>CPM: <strong><?php echo $totaleCpmShow; ?></strong></span> <span class="txspr">·</span><?php } ?> <span>Ganancias: <strong><?php echo $DE . number_format($TotalRevenue, 2, ',', '.') . $DEb; ?></strong></span></th>
										</tr>
										</tfoot>
									</tbody>
								</table>
