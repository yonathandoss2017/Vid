			...
			<div class="bx-bd"><?php
					...
					if(strlen($_POST['email']) > 3){
						if(check_mail($_POST['email'])){
							...
							if($idUser > 0){
								...
								$error = false;
								$errorText = 'Hemos enviado un correo electrónico a su casilla indicando las instrucciones para recuperar su contraseña.';
								$showform = false;
								
								$subject = 'Recuperar contraseña Vidoomy.com';
								...
								*/
								...
								$message .= "<p>Para recuperar la contraseña haz <a href='https://login.vidoomy.com/rec.php?code=$Code'>click aquí</a></p>";
											
								$message .= "<p>Si no te funciona puedes copiar el siguiente enlace y pegarlo en tu navegador: <strong>https://login.vidoomy.com/rec.php?code=$Code</strong></p>";
								$message .= "<p>Este enlance sólo funcionará durante 6 horas.</p>";
								$message .= "</body></html>";
				...
				if($showform){
					?><form action="" method="post" class="frm-login">
						...
						<div class="frm-group">
	                        <input class="md-20 login-btn" type="submit" value="Solicitar contraseña" style="width: 100%" />
	                    </div>
					</form><?php
				}
			?></div>
