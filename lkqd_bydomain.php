<?php	
	...
	$post = array(
		"timeDimension" => "DAILY",
		"reportType" => array("PARTNER", "SITE", "DOMAIN"),
		"reportFormat" => "JSON",
	...
	foreach($decoded_result->data->entries as $entry){
		...
		*/
		...
		if($inserta == 1){
			$sql = "SELECT id FROM " . STATS_DOMAINS . " WHERE idUser = '$idUser' AND idTag = '$idTag' AND Domain = '$Domain' AND Date = '$Date'";
			$idStat = $db->getOne($sql);
			if($idStat > 0){
				$sql = "UPDATE " . STATS_DOMAINS . " SET Impressions = '$Impressions', Opportunities = '$Opportunities', formatLoads = '$formatLoads', Revenue = '$Revenue', Coste = '$Coste', Clicks = '$Clicks', lastUpdate = '$lastUpdate' WHERE id = '$idStat' LIMIT 1";
				$db->query($sql);
			}else{
				$sql = "INSERT INTO " . STATS_DOMAINS . " (idUser, idTag, idSite, Domain, Impressions, Opportunities, formatLoads, Revenue, Coste, Clicks, timeAdded, lastUpdate, Date) VALUES ('$idUser', '$idTag', '$idSite', '$Domain', '$Impressions', '$Opportunities', '$formatLoads', '$Revenue', '$Coste', '$Clicks', '$timeAdded', '$lastUpdate', '$Date')";
				$db->query($sql);
			}
		}
