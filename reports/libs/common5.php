...
function utf8ize($mixed) {
	...
	$DimensionsSQL = array(
		...
		'country' => array(
			...
			'InnerJoin'		=> 	array(
			'reports_country_names' => "INNER JOIN reports_country_names ON reports_country_names.id = $ReportsTable.Country "),
			'GroupBy'		=>	"Country",
			'OrderVal'		=>	"Country",
			'Checked'		=> false,
			'InvalInner'	=> '',
		...
		/*
			'country' => array(
			'Name'	=>	"country_extended.name AS Country",
			'SearchName'	=>	"reports_country_names.idVidoomy",
			'InnerJoin'		=> 	array(
									'reports_country_names' => "INNER JOIN reports_country_names ON reports_country_names.id = $ReportsTable.Country ",
									'country_extended' => "INNER JOIN country_extended ON country_extended.iso = reports_country_names.Code AND country_extended.lang = 'ES' "
								),
			'GroupBy'		=>	"Country",
			'OrderVal'		=>	"Country",
			'Checked'		=> false,
		...
		*/
		'publisher_manager' => array(
			'Name'	=>	"accounts.nameManager AS AccountManager",
			'SearchName'	=>	"accounts.idManager",
			'InnerJoin'		=> 	array(
				//'users' 		=> "INNER JOIN users ON users.id = $ReportsTable.idUser ",
				//'acc_managers'  => "INNER JOIN acc_managers ON acc_managers.id = users.AccM "
				'accounts' 		=> "INNER JOIN accounts ON accounts.idUser = $ReportsTable.idUser "
			),
		...
		'publisher_manager_head' => array(
			'Name'	=>	"accounts.nameLead AS ManagerHead",
			'SearchName'	=>	"accounts.idLead",
			'InnerJoin'		=> 	array(
				'accounts' 		=> "INNER JOIN accounts ON accounts.idUser = $ReportsTable.idUser "
			),
			'GroupBy'		=>	"ManagerHead",
			'OrderVal'		=>	"ManagerHead",
			'Checked'		=> false,
			'InvalInner'	=> '',
			'HeadName'		=> 'Team Lead'
		),
		'publisher_manager_subhead' => array(
			'Name'	=>	"accounts.nameSubLead AS ManagerSubHead",
			'SearchName'	=>	"accounts.idSubLead",
			'InnerJoin'		=> 	array(
				'accounts' 		=> "INNER JOIN accounts ON accounts.idUser = $ReportsTable.idUser "
			),
			'GroupBy'		=>	"ManagerSubHead",
			'OrderVal'		=>	"ManagerSubHead",
			'Checked'		=> false,
			'InvalInner'	=> '',
			'HeadName'		=> 'Sub Team Lead'
		),
		'supply_partner' => array(
			
			//'Name'	=> 	"users.user AS SupplyPartner",
			'Name'			=> 	"concat(users.user, '--',users.id) AS SupplyPartner",
	...
	$TimesSQL = array(
		...
		//	'Name'	=> 	"CONCAT($ReportsTable.Date, '01 00:00:00') AS Month",
			'Name'	=> 	"CONCAT(YEAR($ReportsTable.Date), '-', MONTH($ReportsTable.Date), '-01 00:00:00') AS Month", 
			'SearchName'	=> 	"",
			'InnerJoin'		=> 	"",
			'ShowName'		=> 	"Month",
			'GroupBy'		=>	"Month",
		...
		'daily' => array(
			'Name'	=> 	"CONCAT($ReportsTable.Date, ' 00:00:00') AS Date",
			'SearchName'	=> 	"",
			'InnerJoin'		=> 	"",
			'ShowName'		=> 	"Date",
		...
		'hourly' => array(
			...
			'Name'	=> 	"concat(DATE($ReportsTable.Date), ' ', CONCAT($ReportsTable.Hour,':00:00')) AS HourO, CONCAT($ReportsTable.Date, ' ' ,LPAD($ReportsTable.Hour,2,'0') , ':00:00') AS HourOrder",
	...
	$MetricsSQL = array(
		...
		*/
		'cpm' 					=> array(
			'SQL' 		=>	", round((SUM($ReportsTable.Coste)/SUM($ReportsTable.Impressions) * 1000),2) AS CPM ",
			'SQLCSV' 	=>	", concat('$', FORMAT( ( SUM($ReportsTable.Coste) / SUM($ReportsTable.Impressions) * 1000), 2, 'es_AR') ) AS CPM ",
			'Name'		=>	"CPM",
			'OrderVal' 	=>	"CPM",
			'Base' 		=>	array("Coste","Impressions"),
			'NumberF'	=>	false,
			'HeadName'		=> 'CPM'
		),
		/*
