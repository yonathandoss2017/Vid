<?php	
	...
	require('../config.php');
	require('../db.php');
	require('libs/staging_common5.php');
	...
	if($DimensionsOK){
		...
		if($_SERVER['HTTP_ORIGIN'] == 'https://prod.vidoomy.com'){
			$NewTable = $BaseTable . $StartMonth . "_new";
			
			$MetricsSQL['cpm'] = array(
		...
		if($SOOS != 0){
			...
			$PubManFilter = " AND accounts.idManager = '$UserId' ";
		...
		if($EuroReport){
			//$SQLMetrics = str_replace('Revenue', 'RevenueEur', $SQLMetrics);
			$SQLMetrics = str_replace('Coste', 'CosteEur', $SQLMetrics);
			//$SQLMetrics = str_replace('AS RevenueEur', 'AS Revenue', $SQLMetrics);
			$SQLMetrics = str_replace('AS CosteEur', 'AS Coste', $SQLMetrics);
			
			//$SQLMetrics = str_replace('$', '€', $SQLMetrics);
			
			$FilterEuroUsers = " AND supplytag.currency = 2 ";
		}else{
			$FilterEuroUsers = '';
		}
					...
					foreach($Metrics as $MetricName){
						...
						if($MetricsSQL[$MetricName]['NumberF']){
							...
							if(($MetricSName == 'CTR' || $MetricSName == 'VTR' || $MetricSName == 'ProfitMargin') && floatval(str_replace('%','',$Da[$MetricSName])) == 0 && $CSVResponse === true){
								$Data[$Nd][] = '0,00%';
							}elseif(($MetricSName == 'CPM' || $MetricSName == 'Profit' || $MetricSName == 'Revenue' || $MetricSName == 'RevenueEur' || $MetricSName == 'Coste' || $MetricSName == 'CosteEur') && floatval(str_replace('$','',$Da[$MetricSName])) == 0 && $CSVResponse === true){
								if($EuroReport){
									$Data[$Nd][] = str_replace('$','€',$Da[$MetricSName]);
								}else{
									$Data[$Nd][] = $Da[$MetricSName];//'0,00';
								}
							}else{
