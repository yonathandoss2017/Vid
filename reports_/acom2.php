<?php
	...
	if(isset($_POST['filter'])){		
		if($_POST['filter'] == 'Supply Source'){
			...
			$sql = "SELECT id, TagName FROM supplytag ORDER BY id ASC";
			$query = $db->query($sql);
		...
		if($_POST['filter'] == 'Supply Partner'){
			mysqli_set_charset($db->link, "utf8");
			$sql = "SELECT id, user FROM users ORDER BY id ASC";
			$query = $db->query($sql);
			if($db->num_rows($query) > 0){
				$n = 0;
		...
		if($_POST['filter'] == 'Country'){
			mysqli_set_charset($db->link, "utf8");
			$sql = "SELECT id, Name FROM reports_country_names ORDER BY id ASC";
			$query = $db->query($sql);
			if($db->num_rows($query) > 0){
				$n = 0;
		...
		if($_POST['filter'] == 'Domain'){
			mysqli_set_charset($db->link, "utf8");
			$sql = "SELECT id, Name FROM reports_domain_names ORDER BY id ASC";
			$query = $db->query($sql);
			if($db->num_rows($query) > 0){
				$n = 0;
