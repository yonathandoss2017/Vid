<?php	
	...
	if($db2->num_rows($query) > 0){
		...
		$sql = "SELECT Name FROM acc_managers WHERE id = '$UserId' LIMIT 1";
	...
	if($DimensionsOK){
		...
		if($SOOS != 0){
			$ToFilter['include']['publisher_manager'][] = $UserName;
		}
		...
		if(count($UnionTables) > 1){
			...
			if($Overall || 1==1){
				...
				foreach($UnionTables as $Table){
					...
					$SQLQueryT .= "$Union (SELECT '' $SQLBasesTo FROM $Table INNER JOIN supplytag ON supplytag.id = $Table.idTag $SQLInnerJoinsTo WHERE $Table.Date BETWEEN '$DFrom' AND '$DTo' $SQLWhere $EnvFilter) ";
					$Union = "UNION ALL";
				}    
				$SQLQueryT .= ")  AS R ";
		...
		if(count($UnionTables) > 1){
			...
			if($Overall){
				...
				foreach($UnionTables as $Table){
					$SQLBasesTo = str_replace('{ReportsTable}', $Table, $SQLBases);
					$SQLInnerJoinsTo = str_replace('{ReportsTable}', $Table, $SQLInnerJoins);
					$SQLQuery .= "$Union (SELECT $SQLDimensions $SQLBasesTo FROM $Table INNER JOIN supplytag ON supplytag.id = $Table.idTag $SQLInnerJoinsTo WHERE $Table.Date BETWEEN '$DFrom' AND '$DTo' $SQLWhere $EnvFilter) ";
					$Union = "UNION ALL";
				}    
