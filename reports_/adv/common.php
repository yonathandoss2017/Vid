...
{
    $PubManFilter = " AND (reports.idSalesManager = '$idUser'";
...
function updateUserCards($db2, $idUser, $Date, $Roles, $Admin = false)
{
    ...
    if ($Admin) {
        ...
        if (in_array("ROLE_SALES_MANAGER_HEAD", $Roles)) {
            ...
            INNER JOIN campaign ON campaign.id = reports.idCampaing
            INNER JOIN agency ON campaign.agency_id = agency.id
            WHERE reports.idSalesManager = '$idUser' AND reports.Date BETWEEN '$FirstDay' AND '$LastDate'";
        }
    }
    ...
    if ($Admin) {
        ...
        INNER JOIN campaign ON campaign.id = reports.idCampaing
        INNER JOIN agency ON campaign.agency_id = agency.id 
        WHERE reports.idSalesManager = '$idUser' AND reports.Date = '$Date'";
    }
    ...
    if ($Admin) {
        ...
        if (in_array("ROLE_SALES_MANAGER_HEAD", $Roles)) {
            ...
            INNER JOIN campaign ON campaign.id = reports.idCampaing
            INNER JOIN agency ON campaign.agency_id = agency.id
            WHERE reports.idSalesManager = '$idUser' AND campaign.type = 1 AND reports.Date = '$Date' AND reports.Impressions > 0";
        }
    }
    ...
    if ($Admin) {
        ...
        if (in_array("ROLE_SALES_MANAGER_HEAD", $Roles)) {
            ...
            INNER JOIN campaign ON campaign.id = reports.idCampaing
            INNER JOIN agency ON campaign.agency_id = agency.id
            WHERE reports.idSalesManager = '$idUser' AND campaign.type = 2 AND reports.Date = '$Date' AND reports.Impressions > 0";
        }
    }
