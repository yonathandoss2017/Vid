<?php	
	...
	$sql = "SELECT 
		...
		WHERE stats.Date BETWEEN '2020-03-20' AND '2020-03-30' AND idUser != 108 AND idTag > 0 GROUP BY idTag, Date ORDER BY id ASC";// AND users.currency = 1
	...
	if($db->num_rows($query) > 0){
		while($Row = $db->fetch_array($query)){
			...
			$sql = "SELECT SUM(Coste) AS Coste FROM reports202003 WHERE Date = '$Date' AND idTag = '$idTag'";
			$CosteRepo = round($db->getOne($sql), 3);
			$ReduceTotal = $CosteRepo - $Coste;
			//echo $Coste . ' - ' .  $CosteRepo . ': ' . $ReduceTotal . "\n";
			
			if($ReduceTotal > 0){
				
				
				$sql = "SELECT * FROM reports202003 WHERE Date = '$Date' AND idTag = '$idTag' AND Coste > 0 ORDER BY Coste DESC LIMIT 30";
				$query2 = $db->query($sql);
				$TotalReg = $db->num_rows($query2);
				if($TotalReg > 0){
					$ReduceParcial = round($ReduceTotal / $TotalReg, 3);
					while($Row2 = $db->fetch_array($query2)){
						$idRow = $Row2['id'];
						$CosteP = $Row2['Coste'];
						
						if($Coste > $ReduceParcial){
							$sql = "UPDATE reports202003 SET Coste = Coste - $ReduceParcial WHERE id = $idRow LIMIT 1;";
							//echo $sql . "\n";
							$db->query($sql);
						}else{
							echo "ERROR 2: $Date idTag: $idTag idRow $idRow Debería reducir: $ReduceParcial Coste: $Coste \n";
						}
					}
				}else{
