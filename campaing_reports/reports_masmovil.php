		...
		<nav class="navbar">
			<ul class="menu">
				<li class="has-sub active">
					...
					<ul class="submenu">
						<li<?php echo $active1; ?>>
							<a href="reports_masmovil.php">
								<span class="icon dashboard"></span>
								<span>Cookies Users</span>
							</a>
						</li>
						<li<?php echo $active2; ?>>
							<a href="reports_masmovil.php?d=<?php echo date('Y-m-d'); ?>">
								<span class="icon dashboard"></span>
								<span>Cookies Day</span>
							</a>
						</li>
						<li<?php echo $active3; ?>>
							<a href="reports_masmovil.php?e=1">
								<span class="icon dashboard"></span>
								<span>Events</span>
							</a>
						</li>
							...
							<table id="report-table" class=" panel-table" style="width:100%"><?php
								if($repDay){
									...
									<tbody><?php
										...
										$sql = "SELECT DISTINCT(idSite) FROM `mmocookies` WHERE Date = '$D' AND idSite > 0 ORDER BY idSite ASC";
										$query = $db->query($sql);
										if($db->num_rows($query) > 0){
											while($Site = $db->fetch_array($query)){
												...
												$sql = "SELECT COUNT(id) FROM mmocookies WHERE idSite = '$idSite' AND Date = '$D'";
												$Event1 = $db->getOne($sql);
												
												$sql = "SELECT COUNT(DISTINCT(IP)) FROM mmocookies WHERE idSite = '$idSite' AND Date = '$D'";
									...
									<tbody><?php
										$sql = "SELECT DISTINCT(Date) FROM `mmocookies` ORDER BY Date DESC";
										$query = $db->query($sql);
										if($db->num_rows($query) > 0){
											while($Date = $db->fetch_array($query)){
												$D = $Date['Date'];
												$sql = "SELECT COUNT(id) FROM mmocookies WHERE Event = 1 AND Date = '$D'";
												$Event1 = $db->getOne($sql);
												
												$sql = "SELECT COUNT(DISTINCT(IP)) FROM mmocookies WHERE Event = 1 AND Date = '$D'";
												$Event1U = $db->getOne($sql);
												
												$sql = "SELECT COUNT(id) FROM mmocookies WHERE Event = 2 AND Date = '$D'";
												$Event2 = $db->getOne($sql);
												
												$sql = "SELECT COUNT(DISTINCT(IP)) FROM mmocookies WHERE Event = 2 AND Date = '$D'";
												$Event2U = $db->getOne($sql);
												
												$Per = $Event2 / $Event1 * 100;
												$Per = number_format($Per, 2, ',', '');
												?><tr>
													<td><a href="reports_masmovil.php?d=<?php echo $D; ?>"><?php echo $D; ?></a></td>
													<td><?php echo $Event1; ?></td>
													<td><?php echo $Event1U; ?></td>
