<?php
	...
	*/
	...
	$arDeals = explode('<br />', nl2br(file_get_contents('deal_ids_to_assing.txt')));
	//print_r($arDeals);
	
	foreach($arDeals as $DealID){
		echo $DealID = trim($DealID);
		
		
		$sql = "SELECT id FROM campaign WHERE deal_id = '$DealID' LIMIT 1";
		$idDeal = $db3->getOne($sql);
		...
		$sql = "INSERT INTO account_manager_campaigns (user_id, campaign_id) VALUES (114, $idDeal)";
		$db3->query($sql);
